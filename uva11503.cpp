...
string findP(string v)
{
    if (mp[v].first == v)
        return v;
    else
        return mp[v].first = findP(mp[v].first);
}
void dsu(string v, string u)
{
    if (mp[u].second > mp[v].second)
        swap(u, v);
    mp[u].first = mp[v].first;
    mp[v].second += mp[u].second;
    mp[u].second = 0;
}
void solve()
{
    ...
    while (m--)
    {
        ...
        if (a != b)
            dsu(a, b);
        // cout << a << ' ' << b << endl;
        cout << max(mp[a].second, mp[b].second) << endl;
    }
    mp.clear();
}
int main()
